...
class Image {
     ...
     * @param string $type 图像格式【此处我改成null,并移动参数位置】
     ...
     * @return void
      +----------------------------------------------------------
     */
    static function thumb($image, $thumbname, $maxWidth=200, $maxHeight=50, $type=null, $interlace=true) {
        // 获取原图信息
     ...
     * @return string
      +----------------------------------------------------------
     */
    static function buildImageVerify($type='png', $width=48, $height=22, $randval, $verifyName='verify', $length=4, $mode=1) {
        /*import('ORG.Util.String');
        $randval = String::randString($length, $mode);
        $_SESSION[$verifyName] = md5($randval);*/
        //此处我改成不设置验证码显示的字符串和验证码
        //session名，而是由用户传递参数设定
        //$randval,验证显示的字符串
        //$verifyName,验证码名
        ...
        // 干扰
        for ($i = 0; $i < 10; $i++) {
            imagearc($im, mt_rand(-10, $width), mt_rand(-10, $height), mt_rand(30, 300), mt_rand(20, 200), 55, 44, $stringColor);///画椭圆
        }
        for ($i = 0; $i < 25; $i++) {
            imagesetpixel($im, mt_rand(0, $width), mt_rand(0, $height), $stringColor);///画干扰点(像素)
        }
        for ($i = 0; $i < strlen($randval); $i++) {
            imagestring($im, 5, $i * 10 + 5, mt_rand(1, 8), $randval{$i}, $stringColor);///画字符串
        }
        Image::output($im, $type);
    }

    // 中文验证码
    //此处需要改动个别地方才能用在tp之外的地方
